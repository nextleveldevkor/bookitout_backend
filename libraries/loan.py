...
def search_book_library(keyword):
    ...
    search_button = wd.find_element_by_css_selector(
        "#post-191 > div > div > div > form > div.field-group.text-center > button"
    )
    search_button.click()
    ...
    items = soup.select(
        "#post-193 > div.entry-content > div.content > div > div > div.col-lg-9 > div.items.list > div.item.book"
    )

    for i in range(1, len(items) + 1):
        a = wd.find_element_by_css_selector(
            "div.item-data.d-flex > div.item-meta > div#item-{0}-location > div.item-loc-triggers > a".format(
                i
            )
        )
    ...
    for item in items:
        ...
        title = item.select_one(
            "div.item-data.d-flex > div.item-meta > div.item-title"
        ).text
        author = item.select_one(
            "div.item-data.d-flex > div.item-meta > div.item-author"
        ).text
        link = item.select_one("div.item-data.d-flex > div.item-cover > a").attrs[
            "href"
        ]
        try:
            img_link = item.select_one(
                "div.item-data.d-flex > div.item-cover > a > div"
            ).attrs["style"]
        ...
        cols = item.select(
            "div.item-data.d-flex > div.item-meta > div.item-loc > div.item-loc-contents > div > div > table > tbody"
        )
        for col in cols:
